...
| 已有浮点 Keras 模型 | INT8量化 -> 平台配置 -> Vela 编译 -> 模型集成 |
| 已有 ONNX 模型 | onnx2tf -> FP32 TFLite 基线验证 -> INT8量化 -> 平台配置 -> Vela 编译 |
| 已有INT8量化 TFLite | 平台配置 -> Vela 编译 -> 模型集成 -> 板端验证 |
...
模型转换与量化
    |
    +-- Keras 直转或 ONNX 经 onnx2tf 转换、INT8量化、PC 精度验证
    |
    v
RA8P1 平台配置
    |
